...
function Cyst:SetCystParent(parent, track)
    ...
    local pl = Cyst.kMaxEncodedPathLength
...
function Cyst:TryNewCystParent(parent)
    ...
    local track = TrackYZ():CreateBetween(parent:GetOrigin(), parent:GetCoords().yAxis, self:GetOrigin(), self:GetCoords().yAxis)

    if track then
        Log("%s found better parent %s", self, parent)
        self:ReplaceParent(parent, track)
        return true
    end
    ...
    for i,ent in ipairs(ents) do
        ...
        if self ~= ent and ((ent:isa("Hive") and ent:GetIsBuilt()) or (ent.GetIsActuallyConnected and ent:GetIsActuallyConnected())) then
            ...
            if range < ent:GetCystParentRange() then
                ...
                local track = TrackYZ():CreateBetween(ent:GetOrigin(), ent:GetCoords().yAxis, origin, self:GetCoords().yAxis )
                if track then
                    return ent, track
                end
            end
...
function Cyst:TryToFindABetterParent()
    local parent, track = self:FindBestParent(self, self:GetOrigin(), self:GetCoords().yAxis)

    if parent and track then
        self:ReplaceParent(parent, track)
        return true
    end
...
function Cyst:ReplaceParent(newParent, track)
    ...
    self:ChangeParent(newParent, track)
end


function Cyst:ChangeParent(newParent, track)
    ...
    self:SetCystParent(newParent, track)
...
function Cyst:OnTakeDamage(damage, doer, point)

    // When we take disconnection damage, don't play alerts or effects, just expire silently
    if doer ~= self then
        Structure.OnTakeDamage(self, damage, doer, point)
    end
...
function CreateCyst(player, targetPoint, normal, createMini) 
    ...
    local track = nil
    local cystParent, track = GetCystParentFromPoint(targetPoint, normal, player:GetTeamNumber())
    //Log("Create %s, parent %s, track %s", cyst, cystParent, track)
    if cyst and cystParent and track then    
        connected = true        
    end
